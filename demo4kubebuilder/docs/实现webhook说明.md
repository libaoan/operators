...
```shell
kubectl get secrets webhook-server-cert -n  demo4kubebuilder-system -o jsonpath='{..tls\.crt}' |base64 -d > .certs/tls.crt
kubectl get secrets webhook-server-cert -n  demo4kubebuilder-system -o jsonpath='{..tls\.key}' |base64 -d > .certs/tls.key
```
